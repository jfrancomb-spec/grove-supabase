  ...
  try {
    ...
    const { error: supersedeError } = await adminClient
      ...
      .update({
        ...
        is_live: false,
        status_reason: "Superseded by newly approved job post version",
    ...
    if (supersedeError) {
      return jsonResponse(
        { error: "Failed to supersede current published job post version", details: supersedeError.message },
        500,
      );
    }
    ...
    const { data: approvedVersion, error: approveError } = await adminClient
      ...
      .update({
        ...
        reviewed_at: nowIso,
        reviewed_by: user.id,
        status_reason: notes || "Approved by admin",
        status_changed_at: nowIso,
    ...
    if (approveError || !approvedVersion) {
      return jsonResponse(
        { error: "Failed to approve job post version", details: approveError?.message },
        500,
      );
    }
    ...
    const { error: parentUpdateError } = await adminClient
      ...
      .update({
        ...
        care_type: approvedVersion.care_type,
        location: approvedVersion.location,
        schedule: approvedVersion.schedule,
        description: approvedVersion.description,
        pay_range: approvedVersion.pay_range,
        current_visible_version_id: versionId,
        current_pending_version_id: null,
        is_active: true,
        content_status: "published",
        status_reason: notes || "Published after manual review approval",
        status_changed_at: nowIso,
        status_changed_by: user.id,
        updated_at: nowIso,
      })
    ...
    console.error("approve-job-post-version error", error);
    return jsonResponse(
