...
serve(async (req) => {
  ...
  try {
    ...
    if (versionError) {
      return jsonResponse(
        { error: "Failed to load family profile version", details: versionError.message },
        500,
      );
    }

    if (!version) {
      return jsonResponse({ error: "Family profile version not found" }, 404);
    }

    const familyProfileId = version.family_profile_id;
    if (!familyProfileId) {
      return jsonResponse({ error: "Version is missing family_profile_id" }, 400);
    }
    ...
    if (parentError) {
      return jsonResponse(
        { error: "Failed to load family profile", details: parentError.message },
        500,
      );
    }

    if (!parentProfile) {
      return jsonResponse({ error: "Family profile parent not found" }, 404);
    }
    ...
    if (rejectError || !rejectedVersion) {
      return jsonResponse(
        { error: "Failed to reject family profile version", details: rejectError?.message },
        500,
      );
    }
    ...
    if (!parentProfile.current_visible_version_id) {
    ...
    if (parentProfile.user_id) {
      const remainingFlagged = await countRemainingFlaggedFamilyVersions(adminClient, parentProfile.user_id);
    ...
    console.error("reject-family-profile-version error", error);
    return jsonResponse(
...
async function countRemainingFlaggedFamilyVersions(
  adminClient: ReturnType<typeof createClient>,
  userId: string,
) {
  ...
  const ids =
    (profileRows || []).map((row: { id: string }) => row.id) ||
  ...
  const { count, error } = await adminClient
    ...
    .in("family_profile_id", ids.length ? ids : ["00000000-0000-0000-0000-000000000000"]);

  if (error) {
    throw new Error(`Failed to count remaining flagged family versions: ${error.message}`);
  }
